...
ecs_entity_t CreatePlayer(void)
{
    ...
    PHYSICS_BODY body = {nvBody_new(nvBodyType_DYNAMIC,
                                    nvRectShape_new(s_player.width / SPRITE_SIZE, s_player.height / SPRITE_SIZE),
                                    NV_VEC2(0.0f, 0.0f), 0.0f, nvMaterial_BASIC)};
...
#define PART(id, sprite, x, y, rotation, material, collision)                                                          \
    {                                                                                                                  \
        ...
        PHYSICS_BODY body = {nvBody_new(                                                                               \
            nvBodyType_STATIC, nvRectShape_new((sprite)->width / SPRITE_SIZE, (sprite)->height / SPRITE_SIZE),         \
            NV_VEC2((x) / SPRITE_SIZE, (y) / SPRITE_SIZE), (rotation), (material))};                                   \
        nvSpace_add(g_space, body.body);                                                                               \
